<template>
  <div>
    <AlbumStats :cd-amount=numCDs :digital-amount=numDigital :vinyl-amount=numVinyl :tape-amount=numTape
      @filter-input-changed="updateFilter" />
  </div>

  <table id="album-list-table">
    <thead>
      <th @click="sort('artist')">Artist<div class="arrow" v-if="currentSort == 'artist'"
          :class="sortAscending ? 'arrow-up' : 'arrow-down'"></div>
      </th>
      <th @click="sort('album')">Album<div class="arrow" v-if="currentSort == 'album'"
          :class="sortAscending ? 'arrow-up' : 'arrow-down'"></div>
      </th>
      <th class="th-small" @click="sort('mediatype')">Type<div class="arrow" v-if="currentSort == 'mediatype'"
          :class="sortAscending ? 'arrow-up' : 'arrow-down'"></div>
      </th>
    </thead>
...
export default {
  ...
  methods: {
    ...
    addAlbum(newAlbum) {
      ...
      // probably faster than reloading the database
      this.albumStore['albums'].push(newAlbum)
    },
    selectRow(i) {
